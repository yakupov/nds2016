...
public class IncrementalCorrectnessTest {
    ...
    private void compareIncrementalWithGenerational(final double[][] testData) {
        ...
        for (double[] aTestData : testData) {
            ...
            final double[][] tmpPop = new double[rp.getPop().length][];
            System.arraycopy(rp.getPop(), 0, tmpPop, 0, tmpPop.length);

            final int[] tmpRanks = ppsn2014.performNds(tmpPop);

            try {
                Assert.assertArrayEquals(tmpPop, rp.getPop());
                Assert.assertArrayEquals(tmpRanks, rp.getRanks());
            } catch (Throwable t) {
                System.out.println("Pop: ");
                for (double[] d : rp.getPop()) {
                    System.out.println("\t" + Arrays.toString(d));
                }
                System.out.println("Ranks: " + Arrays.toString(rp.getRanks()));
                throw t;
            }
        }
    }

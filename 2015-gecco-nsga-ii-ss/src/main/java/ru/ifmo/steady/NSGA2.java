...
public class NSGA2 {
    ...
    public Map<Integer, List<Solution>> getFronts() {
        final int layers = storage.getLayerCount();
        final Map<Integer, List<Solution>> res = new HashMap<>();
        for (int layer = 0; layer < layers; ++layer) {
            final Iterator<Solution> f = storage.getLayer(layer);
            final List<Solution> currentLayer = new ArrayList<>();
            while (f.hasNext()) {
                currentLayer.add(f.next());
            }
            if (!currentLayer.isEmpty())
                res.put(layer, currentLayer);
        }
    ...
    public void performIteration() {
        if (variant == Variant.PureSteadyState) {
            for (int i = 0; i < storageSize; ++i) {
                final Solution solution = generateNewSolution();
                ++evaluations;
    ...
    public Solution generateNewSolution() {
        ...
        mutation(cross[0]);
        return problem.evaluate(cross[0]);
    }
